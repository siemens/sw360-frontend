...
export default function LinkedPackages<T extends HasLinkedPackages>({ payload, setPayload }: Props<T>): JSX.Element {
    ...
    const handleComments = useCallback(
        (packageId: string, updatedComment: string) => {
            setPayload((prev) => {
                ...
                for (const [pid, p] of Object.entries(prev.linkedPackages ?? prev.packageIds ?? {})) {
                    if (pid === packageId) {
    ...
    useEffect(() => {
        const data = Object.entries(payload.linkedPackages ?? payload.packageIds ?? {})
        setTableData(data)
    }, [
        payload,
    ])
