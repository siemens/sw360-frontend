 ...
 *
 * Backend reference:
 *   https://github.com/eclipse-sw360/sw360/blob/main/libraries/datahandler/src/main/java/org/eclipse/sw360/datahandler/permissions/DocumentPermissions.java
 */
 ...
 *
 * ```java
 * case USER:         isNormalUser || isAdmin || isClearingAdmin || isEccAdmin || isSecurityAdmin || isSecurityUser
 * case CLEARING_ADMIN: isClearingAdmin || isAdmin
 * case CLEARING_EXPERT: isClearingExpert || isAdmin
 * case ECC_ADMIN:    isEccAdmin || isAdmin
 * case SECURITY_ADMIN: isSecurityAdmin || isAdmin
 * case SW360_ADMIN:  isAdmin
 * case ADMIN:        isAdmin
 * case SECURITY_USER: isSecurityUser
 * ```
 */
 ...
 * Returns `null` when no department matches (equivalent to returning `null` in Java,
 * triggering an early-return `false` in `isUserOfOwnGroupHasRole`).
 */
 ...
 * |---------------------|-----------------------------------------------------------|
 * | READ                | always                                                    |
 * | WRITE / ATTACHMENTS | isUserAtLeast(ADMIN) OR isContributor OR clearingAdminRole |
 * | DELETE/USERS/CLEARING | isAdmin OR isModerator OR adminRole OR clearingAdminRole |
 * | WRITE_ECC           | isAdmin OR adminRole                                      |
 *
 * Backend reference:
 *   https://github.com/eclipse-sw360/sw360/blob/main/libraries/datahandler/src/main/java/org/eclipse/sw360/datahandler/permissions/DocumentPermissions.java
 *
